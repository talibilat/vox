...
def _start(args: argparse.Namespace, config: Config) -> int:
    if args.foreground:
        daemon.run(config)
        return 0
...
def _stop(_args: argparse.Namespace, config: Config) -> int:
    pid = daemon.stop(config)
    print(f"earshot daemon stopped (pid {pid})")
    return 0


def _status(_args: argparse.Namespace, config: Config) -> int:
...
def _interrupt(_args: argparse.Namespace, config: Config) -> int:
    pid = daemon.interrupt(config)
    print(f"interrupt sent to earshot daemon (pid {pid})")
    return 0


def _run(_args: argparse.Namespace, config: Config) -> int:
    daemon.run(config)
    return 0


COMMANDS: dict[str, Callable[[argparse.Namespace, Config], int]] = {
    "start": _start,
    "stop": _stop,
    "status": _status,
